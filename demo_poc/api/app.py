...
def state():
    ...
    if request.method == 'GET':
        ...
        if(states !=[]):
            ...
            response= jsonify({"states":state_list},{'Message':"Successfully retrieved data"})
            response.status_code = 200
            return response
        else:
...
def item_category():
    if request.method == 'GET':
        ...
        if(category !=[]):
            ...
            response= jsonify({"category_list":category_list},{'Message':"Successfully retrieved data"})
            response.status_code = 200
            return response
        else:
...
def item_list():
    if request.method == 'GET':
        ...
        response= jsonify({'item_list' : item_list},{'Message':"Successfully retrieved data"})
        response.status_code = 200
        return response
...
@app.route("/city_filter", methods = ['GET','POST'])
def city_filter():
    # print("Inside")
    ...
    # print(item_id)
        ...
        if(value !=[]):
            for cities in value :
                ...
                response =jsonify({'city_list':city_list},{'Message':"Successfully retrieved data"})   
                response.status_code = 200
...
def brand_filter():
    ...
    if request.method == 'POST':
        ...
        if(brand_list !=[]):
            ...
            response =jsonify({'brand':brand},{'Message':"Successfully retrieved data"})   
            response.status_code = 200
            return response
        else:
...
def price_filter():
    ...
    if request.method == 'POST':
        ...
        if(price_list !=[]):
            ...
            response =jsonify({'item_price':item_price},{'Message':"Successfully retrieved data"})   
            response.status_code = 200
            return response
        else:
...
def update():
    if request.method == 'PUT':
        ...
        pgconn.commit()
        response =jsonify({'Message':"Data successfully Updated in db"})   
        response.status_code = 200
        return response
        pgcursor.close()
        pgconn.close()  
